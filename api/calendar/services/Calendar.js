...
module.exports = {
  ...
  fetchOfUser: async (params, user_id) => {
    ...
    const entry = await Calendar.forge(_.pick(params, 'id')).fetch({
      withRelated: populate
    });

    if (entry && entry.attributes.owner !== user_id) return null;
  ...
  editOfUser: async (params, values, user_id) => {
    ...
    const check_entry = await Calendar.forge(_.pick(params, 'id')).fetch({
      withRelated: populate
    });

    if (!check_entry || !check_entry.attributes || check_entry.attributes.owner !== user_id) return null;
